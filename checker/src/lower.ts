...
export class Lowerer {
  ...
  // fixed up front so a declaration is portable across checker versions. Three
  ...
  private static readonly PROOF_CHECKABLE = new Set(["total", "exhaustive", "handled"]);
  ...
  private lowerProofs(node: N | undefined): string[] {
    ...
    for (const id of node.namedChildren.filter(c => c.type === "lower_id")) {
      ...
      if (!Lowerer.PROOF_CHECKABLE.has(ob)) {
        this.diagnostics.push({ kind: "error", span: this.sp(id),
          message: `proof obligation '${ob}' is not checkable yet — declaring it would promise an unenforced guarantee (checkable today: total, exhaustive, handled)` });
        continue;
      }
      out.push(ob);
    }
