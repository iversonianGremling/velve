...
| **Low-level** | B− | Rust (borrow + sized types); Zig (comptime) | Four numeric stories that never met + gpu/audio/std-low sketches. A+ = **F#-style units-of-measure** as the general mechanism + sized types built on it. See §5. | **Build** — the unifying note is WRITTEN (`numeric-dimension-design.md`, B1 2026-06); grade moves B−→A− when B2 (units) + B3 (sized types + `overflow`) ship the two mechanisms. |
