...
| **Event / state** | A− | Erlang/OTP + Temporal + XState — Velve already unifies all three | Per-stream **backpressure** (`drop \| buffer N \| block` at decl site, TODO §3.2) + fix `await`-to-step-goto parse. Closest row to A+. | **Build** — two small named gaps. |
| **Error handling** | B+ | Rust (`Result`+`?`+`thiserror`); Swift typed `throws`; Zig inferred error sets | B→B+ was readability (ternary deleted). B+→A is **semantic**: named error ADTs (kill the `Result T String` tax). A→A+ is **inferred error rows** — see §4. | **Design + build** — needs a convention *and* row inference. |
