...
| **Low-level** | **A−** *(2026-06)* | Rust (borrow + sized types); Zig (comptime) | Four numeric stories that never met + gpu/audio/std-low sketches. A+ = **F#-style units-of-measure** as the general mechanism + sized types built on it. See §5. | **B−→A− re-graded 2026-06** — both named mechanisms shipped: **B2 units-of-measure** (`unit_test`/`_bad`) and **B3 sized types + the `overflow` obligation** (`sized_test`/`sized_bad`, `proof_overflow_test`/`_bad`), the latter closing the proof vocabulary at 7/7. Held-back **+** is the native representation (real machine `u8` vs f64), Phase D. See §5. |
...
path facts over ℝ — non-constant decreases (`shrink(n - k, k)` under `k >= 1`) prove too,
which no structural rule could. Since 2026-06 `floor(e)` is itself a first-class Int-sorted
solver term, so the `bounds`+`terminates` binary search recursing on `floor(span / 2)` ships
green as the gradient's showcase (`proof_binsearch_test`/`_bad`). And `@total` *pays back into the
...
| | Effects (built) | `proofs: [...]` (mirrors it) |
|---|---|---|
| Per-function | `Effect [network] T` | `proofs: [total, bounds]` body-head clause — **shipped 2026-06** (`proof_fnscope_test`/`_bad`) |
| Per-block | `@unsafe{ … }` | `@proof[total, bounds]{ … }` — **cut for now** (OQ#3; module + per-function cover every fixture) |
| Per-module | `capabilities: [render, network]` | `proofs: [total, bounds]` — **shipped 2026-06** (`proof_scope_test`) |
...
**Surface revision (2026-06):** the per-function scope shipped as a *trailing*
`proofs: [...]` clause at the head of the function body — mirroring the module
body head, the same `proofs_decl` production reused verbatim — rather than the
`Proof [...] T` result brackets sketched above, which would have overloaded the
return-type surface with a set-valued annotation. The set reads better as a
clause (the `effects: [...]` clause shares the shape). The per-block `@proof[…]{}`
form is cut for now (OQ#3).
...
| `overflow` | silent numeric overflow | sized-type width tag + path facts | **SHIPPED 2026-06** (`proof_overflow_test`/`_bad`) — the same two-sided Z3 query as `bounds` against the width range; width-carrying params seed their range, so a guarded op proves and the unguarded one reports the out-of-range model. Closes the vocabulary at 7/7 |
...
obligation. `total`, `exhaustive`, `handled`, `nonzero`, `bounds`, `arith`, and `overflow` are
shipped — **all 7 obligations, the vocabulary complete** as of B3(ii) (2026-06). So
...
`handled` + `nonzero` + `bounds` + `arith` + `overflow` checkable — the vocabulary now 7/7
complete, the rollout above as built. `handled` also surfaced the second enforcement shape:
it is scope-local
...
4. **Every unshipped obligation has a named blocker — and the blockers keep falling on
   ...
   all 7 obligations
   are now checkable (B3(ii), 2026-06 closed `overflow`), exactly the per-obligation cadence
   the rollout promised — the vocabulary is complete.
...
**The unifying note is now written — `numeric-dimension-design.md` (B1, 2026-06).** It pins
this rubric into a buildable design: one algebra where `Number` is the dimensionless case,
units are a new solver-invisible `United` type variant (`type Meters = Number unit m`, NOT
transparent to base so `m * m → Duration²` and `m + s` errors), and sized types are range
refinements over `Number` + an IR width tag (erase-on-JS / native-primitive). Conversions are
explicit-casts-only; the 7th proof word `overflow` rides the existing `bounds` fact-env/Z3
path once the width substrate exists. So the §3.4 🔴 is no longer "unwritten" — it is queued
...
**B3(ii) SHIPPED 2026-06 — Low-level re-grades B− → A−** (`proof_overflow_test`/`_bad`, SPEC §12.7):
axis (1) closes and the proof vocabulary completes at 7/7. Under `proofs: [overflow]` every `+`/`-`/`*`
whose operands carry a width tag runs the SAME two-sided Z3 query as `bounds`, against the width range
(unsigned `[0, 2^bits−1]`, signed `[−2^(bits−1), 2^(bits−1)−1]`) instead of a list length; each
width-carrying param seeds its range (`0 ≤ a ≤ 255` for `a: U8`), so a guarded `if a <= 255 - b then
a + b` proves while the unguarded `a + b` reports the out-of-range model. The two named mechanisms of
the row's own A− definition — units-of-measure (B2) and sized types (B3) — have both shipped; the
held-back **+** is the native representation (real machine `u8` vs an f64), Phase D. As built: the
width tag rides only ascribed params (operand always a name → the proof is relational, the "interval
floor for the literal case" vacuous) and is lost on intermediate results (`(a+b)+c` checks the inner
op only — Phase D's lowering propagates it); the div/unary-minus INT_MIN corner is a documented gap.
The two anticipated re-pins (`proof_scope_bad`/`proof_fnscope_bad`) flipped from the not-checkable
error to a real overflow, exactly as their own comments predicted.
...
- **Live design choice still open** — Type-core (the gradient's *surface*, `@private type`,
  ...
  shipped 2026-06, the return-gate witness spelling too; finer proof scopes shipped per-function 2026-06, per-block cut, §3),
