...
1. **The env holds *values*, not *facts*.** It's `Map<string, ConstVal>` (constants only). A
   ...
   *(The floor of this lift SHIPPED 2026-06 as `facts.ts`: a flow-sensitive fact env of
   ...
   solver as refutation queries over ℝ, with counterexample models in the errors. Still
   future: `len(xs)` symbols, which is what `bounds` needs.)*
...
| 2 | **Z3 `std/proof`** — `proof.sorted`, `proof.terminates`, arbitrary predicates | semantic / termination residue, zero ceremony | **yes** | **first slice BUILT (2026-06)**: the `nonzero` back-end (`smt.ts`, `proof_nonzero_z3_test`/`_bad`) — refutation over ℝ, counterexample models in errors, lazy-loaded, floor fallback when uninstalled; `proof.sorted`/`proof.terminates` remain **proposed stub** (SPEC §std/proof) |
...
5. 🟢 Z3 **Tier-2** + relational witnesses — later, opt-in, for the semantic residue.
   *(Tier 2 is now LIVE for its first obligation (2026-06, two slices): the fact env
   ...
   rollout working at the solver tier. Still open: the semantic residue proper
   (`proof.sorted`, `proof.terminates`) and the relational witness.)*
