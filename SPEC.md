...
`takesU8(300)` is a check error. **The `overflow` obligation reads this tag —
B3(ii) SHIPPED 2026-06** (`proof_overflow_test`/`_bad`, §12.7): under
`proofs: [overflow]` every `+`/`-`/`*` whose operands carry a width must prove
its result in range, closing the proof vocabulary at 7/7.
...
**The `effects: [...]` clause** (shipped 2026-06, `effect_clause_test`/`_bad`).
A function may spell its concrete effect row as a clause at the head of its
body instead of wrapping the return type:

```
def shrink(): ()
  effects: [ui]              -- sugar for `def shrink(): Effect [ui] ()`
  setViewport(#{ width: 375, height: 667, breakpoint: Mobile })
```

It is pure sugar: the clause's capability names *union* into the function's row
and are enforced and propagated identically to an inline `Effect [...]` wrapper
(a body may perform only what the row names; a caller must cover it). The two
forms may co-occur — the inline form stays because it is the only one that can
express an effect *tail* (`Effect [..e]`, the HOF-polymorphism case above),
which a fixed list cannot; when both appear the rows union. The clause is the
same `effects_decl` production used nowhere else, sharing its shape with the
module body head's `capabilities:`/`proofs:` and the per-function `proofs:`
clause (§12.7 A4) — the up-flowing effect clause then the down-flowing proof
clause, mirroring the module body head. Single-line `-> expr` defs have no body
head, so they keep the inline form.
...
**Floored measures** *(2026-06, `proof_binsearch_test`/`_bad`)* — the gradient's
showcase: a recursive binary search green under `proofs: [bounds, total]`, both
obligations on one function. The window is a start `lo` and a length `span`, and
the left half recurses on `floor(span / 2)`. Over the reals that halving does
*not* unit-decrease for `span ∈ [1, 2)` (`span/2 ∈ [0.5, 1)`, not ≤ `span − 1`),
so the measure check now admits `floor(e)` into the solver fragment as a
first-class **Int-sorted** term bracketed by `e − 1 < ⌊e⌋ ≤ e`. Integrality is
the whole point: on `span ∈ [1, 2)`, `⌊span/2⌋` is pinned to `0`, which *is*
≤ `span − 1` — the termination analog of the bounds-soundness argument
(`0 ≤ i < len ⟹ 0 ≤ ⌊i⌋ < len`), and the existing "unit decrease over ℝ above a
floor is finite" soundness covers it unchanged. The `_bad` twin pins the limit:
`if span <= 0` (vs the green's `if span < 1`) leaves the `(0, 1)` gap live where
`⌊span/2⌋ = 0 ≰ span − 1`, and Z3 answers with a fractional `span`. The same
floor term serves `bounds` (a floored index read), `nonzero`, and `arith`.
...
**Construct-implied totality (2026-06, `implied_total_test`/`_bad`).** Some
roles are total by virtue of WHERE they sit — totality comes from the construct,
not a `@total` marker. The pure structural roles get the §12.6 check
automatically:

- **Refinement predicates** — a function called in a `type T = Base where
  pred(value)` clause runs at refinement/check time, so it must terminate (and
  constEval can only fold a predicate whose call-closure is total). The
  predicate `def` joins the total set; the downward gate then forces its closure
  total too. `type Pos = Number where isPositive(value)` checks `isPositive` even
  with no `@total`.
- **Store reducers** — a `messages` handler is a pure state transition, checked
  as if `@total` (it never recurses, so this is just the downward gate: a
  reducer may only call total code).

Effectful roles (machine transitions, inputmap actions) are governed by the
effect system instead, not totality. The game `update` reducer is the third pure
role and stays deferred to Track C with the interaction model. This work also
corrected the HOF-argument check to the data-first convention (`filter(xs, f)`,
not the stale fn-first `filter(f, xs)`): the function argument is validated
wherever it sits, so a reducer's `xs |> filter(f)` is recognized as total.
...
The obligation vocabulary is **closed** — seven fault classes, fixed up front so
a declaration is portable across checker versions: `total`, `bounds`,
`nonzero`/`arith`, `overflow`, `exhaustive`, `handled`. An unknown name is a
compile error. **Declared = enforced**: as of `overflow` (B3(ii), 2026-06) all
seven are checkable, so the vocabulary is now both closed AND complete — the
"not-checkable-yet is an error, never a silent skip" guard remains in the lowerer
as a forward safety net (the next word added to the vocabulary before it is
wired would trip it), but no current word does. A `proofs:` line can never
promise more than the compiler actually verified.
...
- **`handled`** *(2026-06, `proof_handled_test`/`_bad`)* — no def in the module
  ...
  position in a `loop` body) is an error. Matching, `?`-propagation, binding,
  and returning the `Result` are the sanctioned paths — under the declared
  obligation even a *deliberate* discard is rejected; the escape hatch, as
  everywhere in the gradient, is not declaring it. v1 scope: function bodies
  (store messages / machines / sagas are not walked).
- **`nonzero`** *(2026-06, `proof_nonzero_test`/`_bad`)* — every `/` and `%` in
  the module must have a divisor the checker can **prove** nonzero. The runtime
  fault is silent (`x / 0` is JS division — the result poisons with
  `Infinity`/`NaN` instead of crashing), which is exactly why it wants a static
  obligation: there is no error to handle after the fact. The discharge engine
  is the **flow-sensitive fact environment** (north-star §3.1 catch 1):
  comparison-to-constant facts about *immutable names*, collected from the
  branch structure the value already flowed through — `if d == 0 then … else
  <d != 0 here>` (negated facts on the else arm), `&&` conjunctions (`||` only
  when negated), `not`, match literals (`match d | 0 -> … | m -> <m != 0>` —
  the fall-through binder inherits the `!=` of every literal branch above it),
  branch guards, `for` filter clauses, and earlier multi-clause literal heads
  (`def recip(0) -> …` means the `n` of the next clause cannot be 0). A `mut`
  binding never carries facts and reassignment kills them; a fact on an
  immutable name survives into lambdas (the name is frozen at the test). The
  discharge is **two-tier**. Tier one, sync and free: the interval floor — a
  divisor passes iff it is a nonzero literal or a name with a fact among
  `!= 0`, `== k≠0`, `> k≥0`, `≥ k>0`, `< k≤0`, `≤ k<0`. Tier two *(2026-06,
  `proof_nonzero_z3_test`/`_bad`)*: what the floor can't settle but can
  translate (terms over names with `+`/`-`/`*`/unary minus) goes to **Z3** as
  a refutation query over the reals — facts ∧ divisor = 0 unsat ⟹ proved —
  so `if a != b then n / (a - b)`, strict-order guards, one guard covering
  several divisors, and even guard-free nonlinear shapes (`d * d + 1`)
  discharge with zero ceremony; a failed query reports the **counterexample
  model** in the error (`a = 0.0, b = 0.0`). The solver only ever *removes*
  floor errors, never accepts what it can refute; `z3-solver` loads lazily
  (~120 ms, only when a residue exists), and if it isn't installed every
...
- **`arith`** *(2026-06, `proof_arith_test`/`_bad`)* — every call to a
  **domain-restricted math builtin** in the module must have its argument
  ...
  `nonzero`/`bounds`; v1 scope: function bodies.
- **`overflow`** *(2026-06, `proof_overflow_test`/`_bad`)* — the SEVENTH and
  FINAL checkable obligation, which **closes the vocabulary at 7/7**. In a proved
  scope every `+`, `-`, `*` whose operands carry an **IR width tag** (the B3(i)
  sized family `U8…I32`, §3.13) must have its result proved to stay in that
  width's range (unsigned `[0, 2^bits − 1]`, signed `[−2^(bits−1), 2^(bits−1) −
  1]`). JS Numbers never wrap, so an unproved sum silently breaks the sized
  type's invariant — the same "no error to handle" shape as `arith`'s NaN, and
  the same two-sided Z3 query as `bounds`, against the width range instead of a
  list length (result < min OR result > max, both unsat ⟹ in range). Each
  width-carrying param **seeds** its range (a `U8` param assumes `0 ≤ a ≤ 255`,
  the assume side of the gate's guarantee), so a guarded op proves — `if a <=
  255 - b then a + b` discharges, while the unguarded `a + b` reports the
  out-of-range **model** (`a = 255, b = 255`). The width tag rides only ascribed
  params (so the operand is always a name, the result never folds and the floor
  forwards every translatable result to the solver) and is lost on intermediate
  results, so a chained `(a + b) + c` checks only the inner op — Phase D's native
  lowering is where the tag propagates through. Operators that overflow only at
  the lone signed INT_MIN corner (`min / -1`, unary `-min`) are a documented gap.
  Scope-local like `nonzero`/`bounds`/`arith`; v1 scope: function bodies.
...
`nonzero`, `bounds`, `arith`, and `overflow` are scope-local** (their faults are syntactic to the
proved scope; a callee outside the module may still match partially, drop a
`Result`, index unsafely, call `sqrt` on a negative, or overflow a sized add *inside itself*, and that is the callee's unproven
business, exactly as with code called from any verified kernel).

**Finer scope — per-function `proofs: [...]` clause** (shipped 2026-06,
`proof_fnscope_test`/`_bad`). A `proofs: [...]` clause at the head of a function
body promises THAT def's obligations — the module need not declare them. It is
the same `proofs_decl` production reused verbatim, the same closed vocabulary
validated identically, and the same assume/guarantee, scoped to one def: a
sibling without the clause is unconstrained (`loose` does the read `get` must
prove, and is silent because it never opted in). The clause is a *trailing*
head clause, mirroring the module body head — chosen over the originally-
sketched per-def `Proof [total, bounds] T` result brackets, which would have
overloaded the return-type surface; the set-valued obligation list reads better
as a clause (the `effects: [...]` clause shares this shape, §12.4). `total` in a
function clause routes through the totality engine exactly as `@total` would.
The per-block `@proof[...] { }` form remains deferred (north-star §3.4, endgame
OQ#3): the module and per-function scopes have covered every fixture so far.
...
- Z-index and paint order. Tangential but related (note 2026-06-13): formalize a
  *visibility priority* — which elements have prominence when more than one wants the
  user's attention — as a first-class concept rather than a raw z-axis integer. The
  motivating case: opening a modal should let you declare what *matters* in that moment
  and have everything that doesn't matter recede (dim/defocus/inert) automatically,
  instead of hand-tuning z-index and overlay opacity per element. So: a declarative
  prominence ordering (a partial order over elements, or "focus context" that demotes
  the rest), from which paint order, dimming, and pointer-inertness are *derived*. Fits
  the convergence-layer spirit (§6 of styles-design) — state the intent, derive the
  pixels — and connects to the element-state taxonomy (precedence, §12.2 of
  styles-design).
